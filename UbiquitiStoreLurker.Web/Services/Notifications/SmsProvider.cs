...
public sealed partial class SmsProvider(ILogger<SmsProvider> logger) : INotificationProvider
{
    ...
    {
        using var activity = UbiquitiStoreLurkerActivities.Source.StartActivity("notification.send.sms", ActivityKind.Internal);
        ...
        try
        {
            ...
            var message = await MessageResource.CreateAsync(
                body: $"[UbiquitiStoreLurker] {product.Name ?? product.ProductCode} is now {transition.ToState}! {product.Url}",
