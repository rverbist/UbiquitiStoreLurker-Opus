...
public sealed partial class EmailProvider(ILogger<EmailProvider> logger) : INotificationProvider
{
    ...
    {
        using var activity = UbiquitiStoreLurkerActivities.Source.StartActivity("notification.send.email", ActivityKind.Internal);
        ...
        try
        {
            ...
            message.Subject = $"[UbiquitiStoreLurker] {product.Name ?? product.Url} is now {transition.ToState}";
